# Nov 29, 2024 - run this on an H100 to see what happens
...
for i, j in sample_set: 
    ...
    # initialize the model and LoRA setup at each step else the model will be fine-tuned from previous checkpoint 
    ...
    # Enable gradient checkpointing, off loading this to CPU to reduce GPU usage
    ...
    print(f"start to fine-tune {i} _ {j} examples")
    dataset = load_dataset("json", data_files = f"./data/data_5folds/inequality_data_finetuning_sample_{i}_{j}.json", split = "train")
    ...
    trainer = SFTTrainer(
        ...
        args=TrainingArguments(
            per_device_train_batch_size=1, ##maybe batch size can be lower here
            gradient_accumulation_steps=8, ## this could be also lower to reduce memory footprint
            warmup_steps=5,
            max_steps=60,
            learning_rate=2e-4,
    ...
    corpus.to_csv(f'./results/inequality_dataset-gemma2-27b-NoDefinition-FineTuned-{i}-{j}.csv')
    iteration_end_time = time.time()
    iteration_duration = (iteration_end_time - iteration_start_time) / 60
    timing_df = pd.concat([timing_df, pd.DataFrame({'iteration': [(i, j)], 'duration': [iteration_duration]})], ignore_index=True)
    print(f"it takes {iteration_duration} minutes to finetune {i}_{j} sample set")
